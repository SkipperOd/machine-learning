...
logistic_regresson = LogisticRegression(random_state=0)
logistic_regresson.fit(X_train,Y_train)

score = logistic_regresson.score(X_test,Y_test)
print("regression score: ",score)

pred = logistic_regresson.predict(X_test)
...
def plotting(X,Y):
    ...
    plt.contourf(X1, X2, logistic_regresson.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
